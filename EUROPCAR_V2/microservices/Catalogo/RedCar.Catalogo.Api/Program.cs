...
builder.Services.AddDbContext<CatalogoDbContext>(options =>
{
    ...
    {
        options.UseInMemoryDatabase("RedCar.Catalogo.Dev");
    }
});
...
if (!string.IsNullOrWhiteSpace(connectionString))
{
    healthBuilder.AddDbContextCheck<CatalogoDbContext>("database", tags: new[] { "ready" });
}
...
if (enableSwagger)
{
    ...
    app.UseSwaggerUI(c =>
    {
        c.SwaggerEndpoint("/swagger/v1/swagger.json", "RedCar.Catalogo v1");
...
app.Logger.LogInformation("RedCar.Catalogo iniciado en {Urls}", string.Join(",", app.Urls));
app.Run();
