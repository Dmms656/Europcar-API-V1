...
using RedCar.Localizaciones.Api.Extensions;
using RedCar.Localizaciones.Api.Grpc;
...
builder.Services.AddDbContext<LocalizacionesDbContext>(options =>
{
    ...
    {
        options.UseInMemoryDatabase("RedCar.Localizaciones.Dev");
    }
});
...
if (!string.IsNullOrWhiteSpace(connectionString))
{
    healthBuilder.AddDbContextCheck<LocalizacionesDbContext>("database", tags: new[] { "ready" });
}
...
if (app.Environment.IsDevelopment())
{
    ...
    app.UseSwaggerUI(c =>
    {
        c.SwaggerEndpoint("/swagger/v1/swagger.json", "RedCar.Localizaciones v1");
...
app.MapControllers();
app.MapGrpcService<LocalizacionesGrpcService>();
...
app.Logger.LogInformation("RedCar.Localizaciones iniciado en {Urls}", string.Join(",", app.Urls));
app.Run();
