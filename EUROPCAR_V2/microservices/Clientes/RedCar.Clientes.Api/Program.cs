...
using RedCar.Clientes.Api.Extensions;
using RedCar.Clientes.Api.Grpc;
...
var connectionString = builder.Configuration.GetConnectionString("Default")
    ?? Environment.GetEnvironmentVariable("ConnectionStrings__Default__Clientes")
    ?? string.Empty;
...
builder.Services.AddDbContext<ClientesDbContext>(options =>
{
    ...
    {
        options.UseInMemoryDatabase("RedCar.Clientes.Dev");
    }
});
...
if (!string.IsNullOrWhiteSpace(connectionString))
{
    healthBuilder.AddDbContextCheck<ClientesDbContext>("database", tags: new[] { "ready" });
}
...
if (app.Environment.IsDevelopment())
{
    ...
    app.UseSwaggerUI(c =>
    {
        c.SwaggerEndpoint("/swagger/v1/swagger.json", "RedCar.Clientes v1");
