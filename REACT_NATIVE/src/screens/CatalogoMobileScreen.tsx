...
/** Catálogo móvil — lista compacta con chips de categoría. */
export default function CatalogoMobileScreen() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const fleet = await loadAllCatalogVehicles(normalizedLocs, (p) => bookingApi.buscarVehiculos(p));
      setVehiculos(fleet);
    } catch {
      setVehiculos([]);
    } finally {
  ...
  const reservar = (v: VehiculoBooking) => {
    ...
    router.push({
      ...
      params: {
        id: String(id),
        ...(v.idLocalizacion ? { idLocalizacion: String(v.idLocalizacion) } : {}),
      },
    });
  };
      ...
      ListHeaderComponent={
        <View style={styles.header}>
          <View style={styles.hero}>
            ...
            <Text style={styles.heroSub}>Explora toda la flota — eliges fechas y extras al reservar</Text>
          </View>
          ...
          <View style={styles.chips}>
            <Pressable
              style={StyleSheet.flatten([styles.chip, !categoriaFilter ? styles.chipActive : null])}
            ...
            {categorias.map((c) => {
              ...
              return (
                <Pressable
                  ...
                  style={StyleSheet.flatten([styles.chip, categoriaFilter === name ? styles.chipActive : null])}
