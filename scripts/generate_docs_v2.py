...
Servicio EUROPCAR V2 (arquitectura de microservicios + Middleware BFF).

Salida: Contexto/Documentacion_Servicio_Europcar_V2.docx

Mantiene la estetica de V1 (portada, tablas azules, diagramas) con contenido
ampliado para microservicios, orquestacion REST/gRPC y flujos V2.
"""
...
OUTPUT = REPO_ROOT / "Contexto" / "Documentacion_Servicio_Europcar_V2.docx"
IMG_DIR = REPO_ROOT / "Contexto" / "imagenes_origen"
IMG_V2_DIR = REPO_ROOT / "Contexto" / "imagenes documentacion"
SEQ_DIR = REPO_ROOT / "Contexto" / "diagramas_secuencia"
TEAM = "Domenica Arcos, Dana Bahamonde, Dylan Medina, Juan Morales"
...
def arch_v2(name: str) -> Path:
    """Figuras propias de la arquitectura V2."""
    return IMG_V2_DIR / name
...
ENDPOINTS_ADMIN_VEHICULOS = [
    ["GET",    "/api/v1/admin/Vehiculos", "Listado de flota activa.", "JWT", "MS.Catalogo"],
    ["GET",    "/api/v1/admin/Vehiculos/disponibles", "Busqueda con filtros back-office.", "JWT", "MS.Catalogo"],
    ["GET",    "/api/v1/admin/Vehiculos/{id}", "Detalle por id.", "JWT", "MS.Catalogo"],
    ["POST",   "/api/v1/admin/Vehiculos", "Alta de vehiculo.", "ADMIN, AGENTE_POS", "MS.Catalogo"],
    ["PUT",    "/api/v1/admin/Vehiculos/{id}", "Edicion (imagen, precio, estado).", "ADMIN, AGENTE_POS", "MS.Catalogo"],
    ["DELETE", "/api/v1/admin/Vehiculos/{id}", "Baja logica.", "ADMIN", "MS.Catalogo"],
]

ENDPOINTS_ADMIN_LOCALIZ = [
    ["GET",    "/api/v1/admin/Localizaciones", "Listado de sucursales.", "JWT", "MS.Localizaciones"],
    ["GET",    "/api/v1/admin/Localizaciones/{id}", "Detalle de sucursal.", "JWT", "MS.Localizaciones"],
    ["GET",    "/api/v1/admin/Localizaciones/ciudades", "Ciudades para selectores.", "JWT", "MS.Localizaciones"],
    ["POST",   "/api/v1/admin/Localizaciones", "Crear sucursal.", "ADMIN", "MS.Localizaciones"],
    ["PUT",    "/api/v1/admin/Localizaciones/{id}", "Actualizar sucursal.", "ADMIN, AGENTE_POS", "MS.Localizaciones"],
    ["DELETE", "/api/v1/admin/Localizaciones/{id}", "Baja logica.", "ADMIN", "MS.Localizaciones"],
]

ENDPOINTS_PUBLIC_RESERVAS_GUEST = [
    ["POST", "/api/v1/reservas/guest-client", "Upsert de cliente invitado (solo idCliente, esNuevo).", "Publico", "MS.Clientes"],
    ["POST", "/api/v1/reservas", "Crear reserva (SPA / OTA). Orquesta cliente + MS.Reservas.", "Publico", "MS.Clientes + MS.Reservas"],
    ["GET",  "/api/v1/reservas/{codigoReserva}", "Detalle de reserva por codigo.", "Publico", "MS.Reservas"],
    ["PATCH", "/api/v1/reservas/{codigoReserva}/cancelar", "Cancelar reserva externa.", "Publico", "MS.Reservas"],
    ["GET",  "/api/v1/reservas/{codigoReserva}/factura", "Factura asociada.", "Publico", "MS.Reservas"],
]

ENDPOINTS_ADMIN_RESERVAS = [
    ["GET",  "/api/v1/admin/Reservas/cliente/{idCliente}", "Historial del cliente (portal / admin).", "JWT", "MS.Reservas"],
    ["PUT",  "/api/v1/admin/Reservas/{id}/confirmar", "Confirmar + pago + factura transaccional.", "JWT", "MS.Reservas (gRPC)"],
    ["PUT",  "/api/v1/admin/Reservas/{id}/cancelar", "Cancelar con motivo.", "JWT", "MS.Reservas"],
]

ENDPOINTS_BOOKING_V2 = [
    ["GET",  "/api/v2/booking/vehiculos", "Busqueda HATEOAS para integradores.", "API-Key / contrato", "MS.Catalogo"],
    ["GET",  "/api/v2/booking/localizaciones", "Sucursales activas.", "API-Key / contrato", "MS.Localizaciones"],
    ["GET",  "/api/v2/booking/categorias", "Categorias SIPP.", "API-Key / contrato", "MS.Catalogo"],
    ["GET",  "/api/v2/booking/extras", "Extras y tarifas.", "API-Key / contrato", "MS.Catalogo"],
    ["POST", "/api/v2/booking/reservas", "Alta de reserva canal externo.", "API-Key / contrato", "Orquestador"],
]

ENDPOINTS_STUBS = [
    ["*", "/api/v1/Contratos/*", "Check-in/out — stub 501 hasta MS dedicado.", "JWT", "Pendiente"],
    ["*", "/api/v1/Pagos/*", "Listado/registro — datos vacios o 501.", "JWT", "Pendiente"],
    ["*", "/api/v1/Mantenimientos/*", "Taller — stub.", "JWT", "Pendiente"],
    ["*", "/api/v1/Usuarios/*", "Admin usuarios — stub.", "ADMIN", "MS.Seguridad (parcial)"],
]

MICROSERVICES = [
    ["MS.Seguridad", "5101", "RedCar_SeguridadDB", "Login, JWT, usuarios, roles, permisos"],
    ["MS.Catalogo", "5102", "RedCar_CatalogoDB", "Vehiculos, marcas, categorias, extras, inventario"],
    ["MS.Localizaciones", "5103", "RedCar_LocalizacionesDB", "Paises, ciudades, sucursales"],
    ["MS.Clientes", "5104", "RedCar_ClientesDB", "Clientes, conductores, upsert por cedula"],
    ["MS.Reservas", "5105", "RedCar_ReservasDB", "Reservas, pagos, facturas, disponibilidad"],
]

REQUISITOS_FUNCIONALES = [
    ["F1", "Gestion de Vehiculos", "Registro, edicion y baja de la flota fisica (placas, modelos, estados).", "Critica"],
    ["F2", "Gestion de Precios y Categorias", "Configuracion de tarifas por tipo de auto (SUV, Economico, etc.) y temporadas.", "Alta"],
    ["F3", "Gestion de Locaciones (Oficinas)", "Administracion de puntos de retiro y devolucion.", "Alta"],
    ["F4", "Gestion de Contratos (Check-in/Out, PDF)", "Entrega/recepcion del auto y documento legal.", "Critica"],
    ["F5", "Gestion de Pagos (Internos, Depositos)", "Cobros presenciales, depositos y reembolsos.", "Critica"],
    ["F6", "Gestion de Clientes", "Base de conductores que rentan en oficina o web.", "Alta"],
    ["F7", "Reportes de Ocupacion", "Dashboard de disponibilidad y uso de flota.", "Alta"],
    ["F8", "Busqueda y Filtrado", "Autos disponibles por fecha, hora y lugar.", "Critica"],
    ["F9", "Seleccion y Reserva de Vehiculo", "Flujo de intencion de renta.", "Critica"],
    ["F10", "Pago en Linea", "Transacciones con tarjeta en la web (simulado en demo).", "Critica"],
    ["F11", "Gestion de Perfil / Historial", "Datos del usuario y rentas anteriores.", "Media"],
    ["F12", "Consulta Disponibilidad (API Booking)", "Stock y costos para OTA.", "Critica"],
    ["F13", "Sincronizacion de Reservas", "Procesar rentas desde canal externo.", "Critica"],
]

REGLAS_NEGOCIO = [
    "RN-01: No se puede confirmar una reserva con fecha de devolucion menor o igual a la de recogida.",
    "RN-02: No se permite reservar vehiculos en estado operativo no disponible (mantenimiento, taller, alquilado, fuera de servicio).",
    "RN-03: No se permiten solapamientos de reservas activas para el mismo vehiculo en el mismo rango.",
    "RN-04: La cancelacion de reservas finalizadas o ya canceladas esta bloqueada.",
    "RN-05: El cliente solo cancela reservas con fecha de recogida futura.",
    "RN-06: total = subtotal + impuestos + extras + cargo one-way.",
    "RN-07: La confirmacion genera pago y factura de forma transaccional (MS.Reservas).",
    "RN-08: El extra de conductor adicional aplica solo con mas de un conductor activo.",
    "RN-09: El estado ALQUILADO se gobierna por flujos operativos, no por edicion manual.",
    "RN-10: Operaciones sensibles registran trazabilidad de usuario y fecha de modificacion.",
]
...
USE_CASES = [
    {
        ...
        "flujo": [
            ...
            "El frontend envia POST /api/v1/Auth/login al Middleware.",
            "El Middleware delega en MS.Seguridad (validacion en RedCar_SeguridadDB).",
            "MS.Seguridad devuelve JWT, roles e idCliente; el Bus propaga cookie rc_auth.",
            "El frontend redirige al dashboard o portal cliente segun el rol.",
        ],
    ...
    },
    {
        ...
        "flujo": [
            "El cliente abre / o /buscar e ingresa fechas y sucursales.",
            "El Middleware consulta MS.Localizaciones y MS.Catalogo (GET /api/v1/vehiculos).",
            "Revisa detalle GET /api/v1/vehiculos/{id} y disponibilidad /disponibilidad (MS.Reservas).",
            "Entra al wizard /reservar/:id (6 pasos: identificacion, fechas, conductores, extras, pago, confirmacion).",
            "Si es invitado: POST /api/v1/reservas/guest-client (upsert en MS.Clientes).",
            "Confirma con POST /api/v1/reservas (orquestacion REST + gRPC en MS.Reservas).",
            "Recibe codigo de reserva, totales y resumen en pantalla.",
        ],
    ...
    {
        "id": "CU-03b", "nombre": "Reserva como invitado (sin cuenta)",
        "actor": "Visitante sin sesion",
        "precondiciones": "Flujo publico habilitado; vehiculo disponible.",
        "flujo": [
            "El invitado completa el paso Identificacion (cedula, nombres, correo, telefono).",
            "POST /api/v1/reservas/guest-client retorna idCliente (sin exponer PII de terceros).",
            "Completa fechas, conductores, extras y pago simulado en el wizard.",
            "POST /api/v1/reservas persiste la reserva en MS.Reservas.",
            "Se muestra pantalla de confirmacion con codigo de reserva.",
        ],
        "alternativos": "Cedula invalida: errores de validacion Zod. Vehiculo no disponible: mensaje y regreso al catalogo.",
        "postcondiciones": "Cliente creado o reutilizado y reserva PENDIENTE asociada.",
    },
    {
        ...
        "flujo": [
            ...
            "Se invoca PUT /api/v1/admin/Reservas/{id}/confirmar en el Middleware.",
            "MS.Reservas ejecuta confirmacion transaccional (pago + factura + estado CONFIRMADA).",
        ],
    ...
    },
    {
        ...
        "flujo": [
            ...
            "El Middleware valida JWT y propiedad; delega PUT /api/v1/admin/Reservas/{id}/cancelar.",
            "MS.Reservas anula pagos/facturas y cambia estado a CANCELADA.",
        ],
    ...
    },
    {
        ...
        "flujo": [
            "Listar y filtrar flota via GET /api/v1/admin/Vehiculos (proxy MS.Catalogo).",
            "Crear, actualizar o eliminar con POST/PUT/DELETE al Middleware.",
        ],
    ...
    },
    {
        ...
        "precondiciones": "Sesion con rol valido.",
        "flujo": [
            "CRUD sobre /api/v1/admin/Localizaciones (proxy MS.Localizaciones).",
            "Gestion de ciudades asociadas cuando aplica.",
        ],
    ...
    },
    {
        ...
        "flujo": [
            "Acceder a /mi-cuenta, /mis-reservas, /historial con JWT activo.",
            "GET /api/v1/Auth/me enriquece perfil con MS.Clientes.",
            "GET /api/v1/admin/Reservas/cliente/{id} lista reservas del cliente.",
            "Opcional: cancelacion PATCH si la regla RN-05 lo permite.",
        ],
    ...
    },
    {
        ...
        "flujo": [
            "El integrador consume /api/v2/booking/* (contrato HATEOAS) o /api/v1/* legacy.",
            "Consulta vehiculos, localizaciones, categorias y extras via Middleware.",
            "Crea/cancela reservas; el Bus orquesta MS.Clientes y MS.Reservas.",
        ],
...
def build_document() -> Document:
    ...
    add_paragraph(doc, "Servicio EUROPCAR V2", bold=True, size=28,
    ...
    add_paragraph(doc, "Plataforma de renta de vehiculos con arquitectura de microservicios (.NET 10), "
                       "Middleware BFF (Middleware.RedCar), frontend React + Vite y PostgreSQL "
                       "database-per-service (cinco bases RedCar_*).",
    ...
    indices = [
        "1. Introduccion y vision general (incl. evolucion V1 → V2)",
        "2. Glosario",
        "3. Estructura del sistema",
        "4. Arquitectura de microservicios y Middleware BFF",
        "5. Casos de uso, reglas de negocio y requerimientos",
        "6. Bases de datos (database per service)",
        "7. Middleware.RedCar — endpoints y orquestacion",
        "8. Construccion del frontend",
        "9. Seguridad, roles y autorizacion",
        "10. Configuracion, ejecucion y despliegue",
        "11. Requerimientos no funcionales",
        "12. Roadmap y estado de migracion",
        "Anexo A. Diagramas funcionales detallados",
    ]
    for item in indices:
        add_bullet(doc, item)
    ...
    add_paragraph(doc,
        "Servicio EUROPCAR V2 es la evolucion de la plataforma integral de gestion de renta de "
        "vehiculos documentada en V1. Conserva el mismo alcance funcional de negocio — catalogo, "
        "disponibilidad, reservas, contratos, pagos, facturacion, mantenimientos y administracion — "
        "pero descompone el monolito en cinco microservicios de dominio mas un Middleware (BFF) que "
        "orquesta las llamadas REST y gRPC hacia el frontend React y canales OTA.")
    add_paragraph(doc,
        "El SPA sigue consumiendo el contrato legacy /api/v1/* expuesto por Middleware.RedCar; "
        "los integradores externos pueden usar ademas el contrato formal /api/v2/booking/* con "
        "enlaces HATEOAS. Cada microservicio posee su propia base PostgreSQL (database per service); "
        "las referencias entre id_cliente, id_vehiculo e id_localizacion son logicas y se validan "
        "en el Bus antes de persistir.")
    add_paragraph(doc,
        "Esta documentacion consolida arquitectura V2, casos de uso actualizados (incluida reserva "
        "invitado), modelo de datos distribuido, catalogo de endpoints del Middleware, construccion "
        "del frontend y lineamientos de despliegue local y en la nube.")

    add_heading(doc, "1.1. Evolucion respecto a V1", level=2)
    evolucion = [
        ["API monolitica", "Europcar.Rental.Api con esquemas rental/security/audit en una BD", "Referencia en _legacy/EuropcarRental"],
        ["API V2 actual", "Middleware.RedCar + 5 microservicios", "EUROPCAR_V2/microservices + Middleware.RedCar"],
        ["Persistencia", "Una base PostgreSQL", "Cinco bases: RedCar_SeguridadDB, _CatalogoDB, _LocalizacionesDB, _ClientesDB, _ReservasDB"],
        ["Comunicacion interna", "Llamadas in-process / EF directo", "HTTP REST entre servicios; gRPC para CrearReserva transaccional"],
        ["Autenticacion", "JWT emitido en monolito", "JWT emitido por MS.Seguridad; validado en Middleware"],
        ["Reserva invitado", "guest-client bajo admin/reservas", "POST /api/v1/reservas/guest-client (publico, MS.Clientes upsert)"],
    ]
    add_table(doc, ["Aspecto", "V1 (monolito)", "V2 (actual)"], evolucion, widths_cm=[3.5, 6.0, 7.0])
    ...
    glosario = [
        ...
        ["BFF / Bus", "Backend for Frontend: Middleware.RedCar, unico punto de entrada del SPA."],
        ["Microservicio (MS)", "Servicio autonomo con API, logica de negocio y BD propia."],
        ["Database per service", "Patron: cada MS persiste solo en su base PostgreSQL."],
        ["gRPC", "Protocolo binario para operaciones transaccionales (ej. CrearReserva)."],
        ["Upsert", "Crear o actualizar cliente por numero de identificacion (MS.Clientes)."],
        ["Orquestador", "Capa Business del Middleware que coordina varios MS en un flujo."],
    ]
    ...
    # ============ 4. Arquitectura ============
    add_heading(doc, "4. Arquitectura de microservicios y Middleware BFF", level=1)
    add_paragraph(doc,
        "La V2 separa dominios en microservicios .NET 10 con Clean Architecture (Api, Business, "
        "DataManagement, DataAccess). El frontend y las OTA solo hablan con Middleware.RedCar, "
        "que traduce el contrato legacy, valida JWT, enruta a MS via HttpClient y gRPC, y "
        "compone respuestas ApiResponse<T> uniformes. No existe FK entre bases: el Bus valida "
        "referencias logicas (cliente existe, vehiculo activo, sucursal valida) antes de orquestar.")

    add_heading(doc, "4.1. Diagrama de arquitectura V2", level=2)
    arch_path = arch_v2("ArquitecturaRedCarV2.png")
    if not arch_path.exists():
        arch_path = img("figura_17.jpeg")
    add_image(doc, arch_path,
              caption="Figura 1. Arquitectura de microservicios RedCar V2 (Frontend → Middleware → MS → BD).",
              width_cm=15.5)

    add_heading(doc, "4.2. Microservicios y bases de datos", level=2)
    add_table(doc, ["Microservicio", "Puerto dev", "Base de datos", "Responsabilidad"],
              MICROSERVICES, widths_cm=[3.2, 1.8, 4.5, 7.0])

    add_heading(doc, "4.3. Capas del Middleware (BFF)", level=2)
    capas = [
        ["Api", "Controllers V1 legacy y V2 booking; Swagger; health; CORS; JWT Bearer."],
        ["Business", "Orquestadores (Reserva, Marketplace, Factura); FluentValidation; politicas de rol."],
        ["DataManagement", "Mapeo DTO ↔ modelos de integracion con MS."],
        ["DataAccess", "HttpClient REST y clientes gRPC hacia microservicios (sin DbContext propio de negocio)."],
    ]
    add_table(doc, ["Capa", "Responsabilidad"], capas, widths_cm=[4.0, 12.5])

    add_heading(doc, "4.4. REST vs gRPC entre componentes", level=2)
    add_bullet(doc, "REST (linea continua): listados, catalogos, CRUD admin, consultas de disponibilidad.")
    add_bullet(doc, "gRPC (transacciones): CrearReserva en MS.Reservas (cliente + conductores + extras + factura atomica).")
    add_bullet(doc, "El Middleware no persiste datos de negocio; MS.Seguridad emite JWT y MS.Clientes ejecuta upsert.")

    add_heading(doc, "4.5. Stack tecnologico", level=2)
    stack = [
    ...
    # ============ 5. Casos de uso ============
    add_heading(doc, "5. Casos de uso, reglas de negocio y requerimientos", level=1)
    add_paragraph(doc,
        "Se conservan las reglas de negocio y requerimientos funcionales de V1. Los flujos "
        "describen la interaccion Frontend → Middleware → Microservicios. Los diagramas de "
        "secuencia V2 reflejan esta topologia.")

    add_heading(doc, "5.1. Reglas del negocio", level=2)
    for rule in REGLAS_NEGOCIO:
        add_bullet(doc, rule)

    add_heading(doc, "5.2. Requerimientos funcionales", level=2)
    add_paragraph(doc,
        "Tabla consolidada de requerimientos funcionales (F1–F13) del documento base EUROPCAR.docx.")
    add_table(doc, ["ID", "Requerimiento", "Descripcion", "Prioridad"],
              REQUISITOS_FUNCIONALES, widths_cm=[1.2, 4.0, 8.3, 2.0])

    add_heading(doc, "5.3. Diagrama conceptual", level=2)
    add_image(doc, img("figura_02.png"),
              caption="Figura 2. Diagrama conceptual del proyecto.",
              width_cm=15.5)

    add_heading(doc, "5.4. Diagrama general de funciones", level=2)
    add_image(doc, img("figura_03.png"),
              caption="Figura 3. Diagrama general de funciones del sistema.",
              width_cm=15.5)

    add_heading(doc, "5.5. Diagramas de secuencia V2", level=2)
    add_paragraph(doc,
        "Interaccion entre actores, frontend React, Middleware.RedCar y microservicios. "
        "Incluye el flujo CU-03b de reserva invitado y portal cliente (CU-12).")

    secuencias_v2 = [
        ("seq_01_cu_01_iniciar_sesion_v2.png", "Figura 5.1. CU-01 Iniciar sesion (MS.Seguridad)."),
        ("seq_02_cu_03_buscar_reservar_v2.png", "Figura 5.2. CU-03 Buscar vehiculos y disponibilidad."),
        ("seq_03_cu_03b_reserva_invitado_v2.png", "Figura 5.3. CU-03b Reserva invitado sin login."),
        ("seq_04_cu_04_confirmar_reserva_pago_v2.png", "Figura 5.4. CU-04 Confirmar reserva con pago."),
        ("seq_05_cu_12_portal_cliente_v2.png", "Figura 5.5. CU-12 Portal cliente / mis reservas."),
        ("seq_06_cu_08_admin_crud_vehiculos_v2.png", "Figura 5.6. CU-08 CRUD vehiculos admin."),
        ("seq_07_cu_06_checkout_v2.png", "Figura 5.7. CU-06 Check-out (contratos en migracion)."),
    ]
    for fname, caption in secuencias_v2:
        add_image(doc, seq(fname), caption=caption, width_cm=16.0)

    add_heading(doc, "5.6. Detalle de casos de uso", level=2)
    for cu in USE_CASES:
    ...
    # ============ 6. Base de datos ============
    add_heading(doc, "6. Bases de datos (database per service)", level=1)
    add_paragraph(doc,
        "En V2 cada microservicio administra su propio esquema PostgreSQL. El modelo logico "
        "equivale al monolito V1 (esquemas rental, security, audit) pero particionado por dominio. "
        "No hay claves foraneas entre bases: id_cliente, id_vehiculo e id_localizacion son "
        "referencias validadas por el Middleware.")
    add_bullet(doc, "RedCar_SeguridadDB — usuarios, roles, permisos, sesiones (ex security + audit parcial).")
    add_bullet(doc, "RedCar_CatalogoDB — vehiculos, marcas, categorias, extras.")
    add_bullet(doc, "RedCar_LocalizacionesDB — paises, ciudades, localizaciones.")
    add_bullet(doc, "RedCar_ClientesDB — clientes y conductores.")
    add_bullet(doc, "RedCar_ReservasDB — reservas, extras de reserva, conductores de reserva, pagos, facturas.")
    add_paragraph(doc,
        "Scripts de referencia: db/microservices/<servicio>/ y Contexto/redcar_postgres_*.sql. "
        "Las entidades conservan auditoria comun (EsEliminado, FechaRegistroUtc, RowVersion, etc.).")

    add_heading(doc, "6.0. Diagrama Entidad-Relacion (modelo logico V1 de referencia)", level=2)
    add_paragraph(doc,
        "El diagrama ER siguiente representa el modelo consolidado del monolito; en V2 las mismas "
        "tablas se distribuyen segun la tabla de microservicios de la seccion 4.2.")
    add_image(doc, img("figura_18.png"),
              caption="Figura 6.0. Diagrama ER de referencia (particionado por MS en V2).",
              width_cm=16.0)

    add_heading(doc, "6.0.1. Asignacion de tablas por microservicio", level=2)
    asignacion = [
        ["MS.Localizaciones", "paises, ciudades, localizaciones"],
        ["MS.Catalogo", "marcas_vehiculos, categorias_vehiculos, extras, vehiculos, localizacion_extra_stock"],
        ["MS.Clientes", "clientes, conductores"],
        ["MS.Reservas", "reservas, reserva_extras, reserva_conductores, pagos, facturas"],
        ["MS.Seguridad", "usuarios_app, roles, permisos, sesiones, aud_eventos, aud_intentos_login"],
        ["Pendiente MS", "contratos, check_in_out, mantenimientos (aun en stub o legacy)"],
    ]
    add_table(doc, ["Microservicio", "Tablas principales"], asignacion, widths_cm=[4.5, 12.0])
    ...
    add_heading(doc, "7. Middleware.RedCar — endpoints y orquestacion", level=1)
    add_paragraph(doc,
        "Middleware.RedCar (puerto 5200 en desarrollo) expone el contrato /api/v1/* para el SPA "
        "y /api/v2/booking/* para integradores. Los controladores Legacy delegan en microservicios; "
        "los modulos Contratos, Pagos, Mantenimientos y Usuarios pueden responder stub o 501 hasta "
        "completar la migracion.")

    add_heading(doc, "7.1. Convenciones generales", level=2)
    add_bullet(doc, "V1 SPA: /api/v1/... — envelope ApiResponse<T> o LegacyBookingEnvelope.")
    add_bullet(doc, "V2 Booking: /api/v2/booking/... — contrato RedCar V2 con _links HATEOAS.")
    add_bullet(doc, "JWT: validado en el Bus (clave JWT__SecretKey compartida con MS.Seguridad).")
    add_bullet(doc, "Publico sin JWT: vehiculos, localizaciones, categorias, extras, reservas, reservas/guest-client.")
    add_bullet(doc, "Columna MS destino: microservicio que persiste o ejecuta la operacion de negocio.")
    add_bullet(doc, "Health: /health/live en Middleware y en cada MS; Swagger en /swagger.")
    ...
    add_heading(doc, "7.5. Reservas publicas e invitado", level=2)
    add_table(doc, ["Metodo", "Ruta", "Descripcion", "Auth", "MS destino"],
              ENDPOINTS_PUBLIC_RESERVAS_GUEST, widths_cm=[1.6, 4.8, 5.2, 1.8, 2.1])

    add_heading(doc, "7.5b. Booking publico — Reservas (alias legacy)", level=2)
    add_table(doc, ["Metodo", "Ruta", "Descripcion", "Autorizacion"], ENDPOINTS_BOOKING_RES, widths_cm=[2.0, 5.5, 6.5, 2.5])

    add_heading(doc, "7.6. Admin — Vehiculos (proxy MS.Catalogo)", level=2)
    add_table(doc, ["Metodo", "Ruta", "Descripcion", "Auth", "MS destino"],
              ENDPOINTS_ADMIN_VEHICULOS, widths_cm=[1.6, 4.5, 5.5, 1.8, 2.1])

    add_heading(doc, "7.7. Admin — Localizaciones (proxy MS.Localizaciones)", level=2)
    add_table(doc, ["Metodo", "Ruta", "Descripcion", "Auth", "MS destino"],
              ENDPOINTS_ADMIN_LOCALIZ, widths_cm=[1.6, 4.5, 5.5, 1.8, 2.1])

    add_heading(doc, "7.8. Admin — Reservas", level=2)
    add_table(doc, ["Metodo", "Ruta", "Descripcion", "Auth", "MS destino"],
              ENDPOINTS_ADMIN_RESERVAS, widths_cm=[1.6, 4.8, 5.2, 1.8, 2.1])
    ...
    add_heading(doc, "7.17. Contrato Booking V2 (/api/v2/booking)", level=2)
    add_table(doc, ["Metodo", "Ruta", "Descripcion", "Auth", "MS destino"],
              ENDPOINTS_BOOKING_V2, widths_cm=[1.6, 4.5, 5.0, 2.0, 2.4])

    add_heading(doc, "7.18. Modulos en migracion (stubs)", level=2)
    add_paragraph(doc,
        "Hasta disponer de microservicios dedicados, estas rutas pueden devolver listas vacias, "
        "501 Not Implemented o delegar parcialmente en _legacy:")
    add_table(doc, ["Metodo", "Ruta", "Descripcion", "Auth", "Estado"],
              ENDPOINTS_STUBS, widths_cm=[1.2, 4.5, 5.5, 1.8, 2.5])
    ...
    rutas = [
        ...
        ["/reservar/:id", "ReservarPage", "Wizard 6 pasos: identificacion, fechas, conductores, extras, pago, confirmacion (invitado o autenticado)."],
    ...
    add_heading(doc, "8.6. Reserva invitado y normalizacion de datos", level=2)
    add_bullet(doc, "Paso Identificacion visible para usuarios sin sesion; POST /reservas/guest-client antes de confirmar.")
    add_bullet(doc, "sessionStorage guarda guestClientId solo tras respuesta exitosa del servidor (no al teclear cedula).")
    add_bullet(doc, "bookingNormalize.js unifica marca/categoria como string (PascalCase/camelCase del middleware).")
    add_bullet(doc, "useAuthStore garantiza roles como array para evitar errores en ProtectedRoute.")

    add_heading(doc, "8.7. Patrones de UI", level=2)
    add_bullet(doc, "Layouts: MainLayout (admin) y ClienteLayout (portal cliente), ambos con Navbar unica en App.")
    ...
    add_paragraph(doc,
        "MS.Seguridad emite el JWT en login; Middleware.RedCar lo valida en cada peticion protegida "
        "(sin almacenar sesiones de negocio propias). Los controladores aplican [Authorize] y "
        "[Authorize(Roles=...)]. Cookie rc_auth (HttpOnly) complementa Bearer en sessionStorage para el SPA.")
    ...
    add_paragraph(doc, "Microservicios (desde raiz del repo):", bold=True)
    add_bullet(doc, "pwsh ./EUROPCAR_V2/scripts/run-all.ps1 — levanta MS en puertos 5101–5105.")
    add_bullet(doc, "dotnet run --project Middleware.RedCar/src/Middleware.RedCar.Api — puerto 5200.")
    add_paragraph(doc, "Swagger Middleware: /swagger ; Health: /health/live. Monolito legado opcional: _legacy/EuropcarRental.")
    ...
    # ============ 12. Roadmap ============
    add_heading(doc, "12. Roadmap y estado de migracion", level=1)
    add_table(doc, ["Modulo", "Estado V2", "Notas"], [
        ["Auth / login / registro", "Operativo", "MS.Seguridad + Middleware AuthController"],
        ["Catalogo / vehiculos / extras", "Operativo", "CRUD admin via MS.Catalogo"],
        ["Localizaciones / ciudades", "Operativo", "CRUD admin via MS.Localizaciones"],
        ["Clientes / conductores", "Operativo", "Upsert y CRUD via MS.Clientes"],
        ["Reservas / pagos / facturas", "Operativo", "Crear, confirmar, cancelar, historial cliente"],
        ["Reserva invitado web", "Operativo", "guest-client + wizard 6 pasos"],
        ["Contratos check-in/out", "En migracion", "Stub 501 o legacy"],
        ["Pagos / mantenimientos admin", "En migracion", "Stubs"],
        ["Usuarios admin write", "En migracion", "Lectura parcial MS.Seguridad"],
        ["Booking V2 HATEOAS", "Parcial", "/api/v2/booking/* implementado en Middleware"],
    ], widths_cm=[4.5, 2.5, 9.5])
    add_paragraph(doc, "Mejoras propuestas:", bold=True)
    add_bullet(doc, "MS dedicados para Contratos y Mantenimientos con eventos de dominio.")
    add_bullet(doc, "OpenTelemetry y correlationId de punta a punta (FE → Bus → MS).")
    add_bullet(doc, "Cache de catalogos en Middleware para latencia < 500 ms (RNF-04).")
    add_bullet(doc, "Pipeline CI: xUnit en MS + Vitest/Playwright en frontend.")
    add_bullet(doc, "Publicar coleccion Bruno/Postman por entorno desde OpenAPI exportado.")
    ...
    add_divider(doc)
    add_paragraph(doc,
        f"Documento V2 generado automaticamente — Equipo: {TEAM}. "
        "Basado en Documentacion_Servicio_Europcar_V1 y arquitectura implementada en Europcar-API-V1.",
